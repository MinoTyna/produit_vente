...
class ListAchatAPIView(APIView):
    def get(self, request, client_id):
        achats = Achat.objects.filter(client_id=client_id).order_by('-date')
        serializer = AchatReadSerializer(achats, many=True)

        # Calcul manuel du total
        total = sum(
            achat.produit.Produit_prix * achat.Achat_quantite for achat in achats
        )
...
class FactureHistoriqueView(APIView):
    def get(self, request, pk):
        ...
        # Filtrer uniquement les achats faits à la même date+heure que la facture
        date_achat = facture.achat.Achat_date.replace(second=0, microsecond=0)
        achats = Achat.objects.filter(
            ClientID=client,
            Achat_date__gte=date_achat,
            Achat_date__lt=date_achat + timedelta(minutes=1)
        )
        ...
        for paiement in paiements:
            total_paye_cumul += paiement.Paiement_montant
            reste = max(prixtotalproduit - total_paye_cumul, Decimal('0'))
            revenu = max(total_paye_cumul - prixtotalproduit, Decimal('0'))  # ✅ revenu
            statut = "complet" if reste == 0 else "incomplet"
            ...
            historique_etats.append({
                ...
                "total_paye": float(total_paye_cumul),
                "reste_a_payer": float(reste),
                "revenu": float(round(revenu, 2)),  # ✅ Ajouté ici
                "statut": statut,
                "paiement_detail": {
...
class FactureAllView(APIView):
    def get(self, request):
        ...
        ).prefetch_related('achat__paiements_details')

        grouped_factures = defaultdict(list)
        now = timezone.now()
        ...
        for facture in factures:
            ...
            client = achat.ClientID
            responsable = achat.ResponsableID
            paiements = achat.paiements_details.all().order_by('Paiement_datechoisi')
            ...
            total_paye = sum(p.Paiement_montant for p in paiements)
            reste = max(prixtotalproduit - total_paye, Decimal('0'))
            revenu = max(total_paye - prixtotalproduit, Decimal('0'))
            statut = "complet" if reste == 0 else "incomplet"

            achat_datetime_str = achat.Achat_date.strftime('%Y-%m-%d %H:%M:%S')

            # 🔍 Prochaine date de paiement choisie dans le futur
            prochaine_date_paiement = None
            for paiement in paiements:
                date_choisie = paiement.Paiement_datechoisi
                if date_choisie and date_choisie > now.date():
                    prochaine_date_paiement = date_choisie.strftime('%Y-%m-%d')
                    break
            ...
            facture_data = {
                ...
                "total_paye": float(total_paye),
                "reste_a_payer": float(reste),
                "revenu": float(round(revenu, 2)),
                "statut": statut,
                "prochaine_date_paiement": prochaine_date_paiement,
            }

            grouped_factures[achat_datetime_str].append(facture_data)

        response_data = []
        for group in grouped_factures.values():
            response_data.extend(group)
